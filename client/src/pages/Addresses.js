  ...
  return(
    <div className="h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        {!addresses ? (<p>Loading</p>) : (
            <>
            <h1 className="text-center m-4 text-2xl font-bold dark:text-gray-200">Addresses From {datasetInfo.name} Dataset</h1>
            <div className="overflow-x-auto container flex md:justify-center mx-auto">
                <table className="divide-y divide-gray-300 ">
                  <thead className="bg-gray-50 dark:bg-slate-600">
                    <tr>
                      <th className="px-6 py-2 text-xs dark:text-gray-200">Street Number</th>
                      <th className="px-6 py-2 text-xs dark:text-gray-200">Route</th>
                      <th className="px-6 py-2 text-xs dark:text-gray-200">City</th>
                      <th className="px-6 py-2 text-xs dark:text-gray-200">State</th>
                      <th className="px-6 py-2 text-xs dark:text-gray-200">Edit</th>
                    </tr>
                  </thead>
                  {addresses.map((address) => (
                    <AddressTable key={address._id} address={address} dataset_info={datasetInfo} />
                  ))}
                </table>
            </div>
            </>
